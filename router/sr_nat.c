...
void insertNATConnection(struct sr_nat_mapping * mapping, uint8_t* buf, uint32_t ip_conn,
	...
	if (conn_state == UN_SYN){
        	new_conn->buf = malloc(ETHE_SIZE + ICMP_DATA_SIZE);
        	memcpy(new_conn->buf, buf,  ETHE_SIZE + ICMP_DATA_SIZE);
	}
...
int processNATConnection(struct sr_nat *nat, uint8_t* buf, struct sr_nat_mapping * mapping,
	...
	if (!found){
		if (tcp_flag == FLAG_SYN){
			if (isClient){
				insertNATConnection(mapping, buf, ip_conn, aux_conn, SYN);
			} else {
				insertNATConnection(mapping, buf, ip_conn, aux_conn, UN_SYN);
				pthread_mutex_unlock(&(nat->lock));
				return 1;
			}
		}
