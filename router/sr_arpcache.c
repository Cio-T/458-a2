...
void sr_arpcache_sweepreqs(struct sr_instance *sr) {
	struct sr_arpreq *this, *next;
	this = sr->cache.requests;
    while (this) {
	...
	if (difftime(now, req->sent) > 1.0){
		if (req->times_sent >= 5){
			...
			while (this_pac){
				...
				int len = ETHE_SIZE + IP_SIZE + ICMP3_SIZE + ICMP_DATA_SIZE;
				makeAndSendICMP(len, this_pac->buf, sr, this_pac->iface, 3, 1);
				this_pac = this_pac->next;
			}
            sr_arpreq_destroy(cache, req);
		} else {
            /*send arp request*/
			sendARPReq(ETHE_SIZE + ARP_SIZE, broadcast_addr, req->ip, sr, this_pac->iface);
            req->sent = now;
			++req->times_sent;
		}
	}
...
void sr_process_arpreply(struct sr_instance* sr,
	...
	if (req) {
		...
		while (this_pac){
			...
			struct sr_if* out_if = sr_get_interface(sr, this_pac->iface);
            prepEthePacketFwd(this_pac->buf, mac, out_if->addr);
